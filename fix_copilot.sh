#!/bin/bash
cat << 'INNER_EOF' > src/lib/api/copilot.functions.ts
import { createServerFn } from "@tanstack/start";
import { z } from "zod";
import { getDatabaseSnapshotFn } from "./database.functions";
import type { DatabaseState } from "../db/types";

function buildContextSnapshot(db: DatabaseState): string {
  const shipments = db.shipments.map(s => \`Shipment \${s.shipmentId} (\${s.status}): ETA \${s.predictedEta}, Risk \${s.riskScore}/100, Delay \${s.estimatedDelayMinutes}m. Factors: \${s.riskFactors?.join(", ") || "None"}\`).join("\n");
  const alerts = db.alerts.map(a => \`Alert \${a.alertId} (\${a.severity}): \${a.description}\`).join("\n");
  const incidents = db.incidents.map(i => \`Incident \${i.incidentId} (\${i.severity}): \${i.cause}\`).join("\n");
...
\`;
    ...
    const criticalShipments = liveDatabaseSnapshot.shipments.filter(s => s.riskLevel === 'CRITICAL' || s.riskScore >= 75);
    ...
    if (
      ...
      const delayedCount = liveDatabaseSnapshot.shipments.filter(s => s.status === 'DELAYED').length;
      return {
